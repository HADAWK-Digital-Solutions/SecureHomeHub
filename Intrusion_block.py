...
class ImprovedIDS:
    ...
    def block_ip(self, ip_src):
        # Function to log the IP for blocking
        alert_msg = f"Blocking IP: {ip_src}"
        print(alert_msg)
        logging.info(alert_msg)

        # Insert the appropriate command to interact with your firewall here.
        # Example for iptables (requires superuser privileges):
        os(f"iptables -A INPUT -s {ip_src} -j DROP")
        ...
        if packet.haslayer(IP):
            ...
            if packet.haslayer(ICMP):
                ...
                if self.icmp_sources[ip_src] > self.threshold_icmp:
                    ...
                    print(alert_msg)
                    logging.info(alert_msg)
                    self.block_ip(ip_src)  # Block IP if threshold is exceeded
